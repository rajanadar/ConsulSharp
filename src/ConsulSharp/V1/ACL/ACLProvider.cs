...
namespace ConsulSharp.V1.ACL
{
    internal class ACLProvider : IACL
    {
        ...
        public async Task<ConsulResponse<string>> UpdateTokenAsync(ConsulRequest<TokenRequestModel> request)
        {
        ...
        public async Task<ConsulResponse<ReplicationStatusModel>> CheckReplicationAsync(ConsulRequest<string> request = null)
        {
            return await _polymath.MakeConsulApiRequest<ReplicationStatusModel>(request, "v1/acl/replication" + ((request != null && !string.IsNullOrWhiteSpace(request.RequestData)) ? ("?dc=" + request.RequestData) : string.Empty), HttpMethod.Get).ConfigureAwait(_polymath.ConsulClientSettings.ContinueAsyncTasksOnCapturedContext);
        }
    }
}
